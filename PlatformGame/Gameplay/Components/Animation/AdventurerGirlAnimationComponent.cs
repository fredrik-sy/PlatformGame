namespace PlatformGame.Gameplay.Components.Animation
{
    ...
    [Serializable]
    internal class AdventurerGirlAnimationComponent : IAnimationComponent
    {
        ...
        public AdventurerGirlAnimationComponent()
        {
        }
        ...
        public void Draw(GameObject gameObject, SpriteBatch spriteBatch)
        {
            ...
            spriteBatch.Draw(_texture, Camera.WorldToScreen(gameObject.Position), null, Color.White, 0, Vector2.Zero, 1, _effects, layerDepth);
        }

        public void LoadContent(Game game)
        {
            _textures = new Texture2D[5][];
            _textures[(int)AnimationState.Dead] = new Texture2D[10];
            _textures[(int)AnimationState.Idle] = new Texture2D[10];
            _textures[(int)AnimationState.Jump] = new Texture2D[10];
            _textures[(int)AnimationState.Shoot] = new Texture2D[3];
            _textures[(int)AnimationState.Walk] = new Texture2D[8];
            ...
            foreach (AnimationState animationState in Enum.GetValues(typeof(AnimationState)).Cast<AnimationState>())
            {
                ...
                {
                    _textures[(int)animationState][i] = game.Content.Load<Texture2D>("AdventurerGirl\\" + animationState.ToString() + "\\" + i);
                }
            }
